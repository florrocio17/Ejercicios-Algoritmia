...
def tirar_carta_jugador(cartas_jugadores):
    ...
    return carta - 1  # Retorna el índice de la carta seleccionada
...
def tirar_carta_computadora(cartas_jugadores, carta_jugador):
    if carta_jugador == 0:  # Es la primera jugada
        ...
        if cartas_validas:  # Si tiene una carta mayor, elige la mayor
            carta_computadora = max(cartas_validas, key=lambda x: x[0])
        else:  # No tiene carta mayor
            # La computadora decide ir al mazo o tirar una carta aleatoria
            if random.choice([True, False]):
                print("Computadora: Me voy al mazo")
                return -1  # Indica que se fue al mazo
            else:
                carta_computadora = random.choice(cartas_jugadores[1])

    print("Computadora: Tira la carta", carta_computadora)
    # Retorna el índice de la carta tirada por la computadora
    return cartas_jugadores[1].index(carta_computadora) + 1


def valor_carta(carta):
    valor = carta[0]  # Obtener el valor numérico de la carta

    jerarquia = {
        # El 1 suele tener el valor más alto, en algunos casos 1 (Ancho) es la carta más alta
        1: 14,
        2: 13,
        3: 12,
        10: 11,
    ...
    # Si el valor está en la jerarquía, devuelve su valor asociado, de lo contrario devuelve el valor original
    return jerarquia.get(valor, valor)


def sistema_de_tirada(cartas_jugadores, puntos_jugador, puntos_contrincante, rondas_jugadas):
    cartas_jugadas = [[], []]  # Cartas jugadas por el jugador y la computadora
    ...
    if carta_computadora == -1:  # La computadora se fue al mazo
        puntos_contrincante += 1
        return puntos_jugador, puntos_contrincante, True
    else:
        cartas_jugadas[1].append(cartas_jugadores[1][carta_computadora - 1])
        cartas_jugadores[0].pop(carta_jugador)
        cartas_jugadores[1].pop(carta_computadora - 1)
        print()
        if rondas_jugadas == 0:
            if valor_carta(cartas_jugadas[0][0]) > valor_carta(cartas_jugadas[1][0]):
                puntos_jugador += 1
            else:
                puntos_contrincante += 1
        elif rondas_jugadas == 1:
            if valor_carta(cartas_jugadas[0][0]) > valor_carta(cartas_jugadas[1][0]):
                puntos_jugador += 1
            else:
                puntos_contrincante += 1
        elif rondas_jugadas == 2:
            if valor_carta(cartas_jugadas[0][0]) > valor_carta(cartas_jugadas[1][0]):
                puntos_jugador += 1
            else:
                puntos_contrincante += 1
        return puntos_jugador, puntos_contrincante, False
...
def main():
    ...
    while accion != -1:
        if accion == 1:
            ...
            puntos_jugador, puntos_contrincante, salir = sistema_de_tirada(
